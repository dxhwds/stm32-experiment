...
//	int d =100;    //单次调节转速
	...
	{	
		if (dir ==0)
		{	GPIO_SetBits(GPIOA,GPIO_Pin_1);GPIO_ResetBits(GPIOA,GPIO_Pin_2);GPIO_SetBits(GPIOA,GPIO_Pin_3);GPIO_SetBits(GPIOA,GPIO_Pin_4);
		delay_ms(SPEED);
			
					GPIO_SetBits(GPIOA,GPIO_Pin_1);GPIO_SetBits(GPIOA,GPIO_Pin_2);GPIO_SetBits(GPIOA,GPIO_Pin_3);GPIO_ResetBits(GPIOA,GPIO_Pin_4);
		delay_ms(SPEED);
		
			GPIO_ResetBits(GPIOA,GPIO_Pin_1);GPIO_SetBits(GPIOA,GPIO_Pin_2);GPIO_SetBits(GPIOA,GPIO_Pin_3);GPIO_SetBits(GPIOA,GPIO_Pin_4);
		delay_ms(SPEED);
			
			
		GPIO_SetBits(GPIOA,GPIO_Pin_1);GPIO_SetBits(GPIOA,GPIO_Pin_2);GPIO_ResetBits(GPIOA,GPIO_Pin_3);GPIO_SetBits(GPIOA,GPIO_Pin_4);
		delay_ms(SPEED);
		...
		if(hw_jsbz==1)	//如果红外接收到
		{
			...
			switch(hw_jsm)
		{
			case 0x00FFA25D:printf("POWER");printf("Key:%x, cnt:%d\n", hw_jsm, hw_jsbz);	dir = 2; ccr=0;				break;   
			...
			case 0x00FF906F:printf("vol+");printf("Key:%x, cnt:%d\n", hw_jsm, hw_jsbz);    SPEED=SPEED-10;  printf("步进电机速度：%d \n",SPEED);      break;		    
			case 0x00FF6897:printf("0");printf("Key:%x, cnt:%d\n", hw_jsm, hw_jsbz);ccr=0;              							break;		  
			case 0x00FFA857:printf("vol-");printf("Key:%x, cnt:%d\n", hw_jsm, hw_jsbz);    SPEED=SPEED+10;   printf("步进电机速度：%d \n",SPEED);printf("速度：%d \n",SPEED);        	break;	   
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  //接收中断
	{
		...
		hw_jsbz=1;
          ...
          if(isupper(hw_jsm))
			...
			if(hw_jsm == '+')	SPEED=SPEED-1;
			if(hw_jsm == '-')	SPEED=SPEED+1;
			if(hw_jsm == 'a')	SPEED=SPEED-10;
			if(hw_jsm == 'd')	SPEED=SPEED+10;	
